...
**Architect vendor 스위치**: 기본 페어링은 Claude=Architect다. Claude Code 없이 Codex 단독으로 쓰려면 인터랙티브 Codex에서 Architect/Reviewer를 맡고, `content/routing.toml`의 `[routing].preset`을 `"codex_only"`로 선택한 뒤 `py -3 install.py --target codex --allow-live`로 설치한다. `~/.codex/orchestrate.py`와 routing 설정이 함께 설치된다(ADR-0013). 기본 `hybrid`는 HIGH Challenger가 Claude이므로 설치만으로 Codex 단독 구성이 완성되지는 않는다. 일회성 선택은 `challenge`와 `build` 양쪽에 `--routing-preset codex_only`를 붙인다. `--builder codex`만으로 Challenger는 바뀌지 않는다. HIGH는 Codex Challenger → Architect 판단 → Codex Builder → Reviewer → 사람 승인 순서를 유지하며, preset은 실행 중인 인터랙티브 세션의 모델을 변경하지 않는다. Claude 단독은 `claude_only` preset을 사용한다(ADR-0020).
