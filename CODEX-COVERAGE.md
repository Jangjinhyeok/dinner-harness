...
> **※ 2026-06 재-recon 정정 (D3·D4·D6 — §1 매트릭스 및 §3 dropped 인벤토리 공통):** 위 "Codex native subagent 실행 메커니즘 없음"(D3)·"enforcement 부재"(D4)·"Codex 대응 기제 없음"(D6) 및 §3의 "Codex subagent exec 부재"·"Codex hook 부재" 류는 **Cycle 2 build-time(Codex 0.111.0) 기준**이다. 현행 Codex는 이후 **hooks·custom agents를 추가**했다 — 따라서 드롭은 *아키텍처 불가*가 아니라 **설계 결정**이다: hooks는 포팅 가능(contract 일치), agents/rules/roles는 **시맨틱 한계**(custom agents = 명시호출·max_depth=1·hub-leaf fan-out 부재; Two-CLI 세션페어·조건부 inject 부재)로 드롭 유지. 상세·매트릭스 = `CODEX-RECON.md` "2026-06 재-recon 갱신".
