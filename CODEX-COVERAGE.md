...
| **D3** | `content/agents/**` (21) | **native** (adapter v2, Cycle 3) | `adapters/codex.py`가 21개를 `~/.codex/agents/*.toml`(name·description·developer_instructions)로 변환 (아래 Cycle 3 노트·전체 목록 §3) |
...
| **D8** | `content/skills/*` (26) | **8 clean + 10 degraded copy / 8 dropped** | §2 상세 |
...
> - **preflight 반영 (CODEX-PREFLIGHT.md, 2026-06-19) + 배선**: ① **apply_patch 배선 — 완료·검증.** Codex 편집은 `tool_name=apply_patch` + `tool_input.command`(패치 envelope), Claude식 `file_path` 아님(§3). `lib/common.parse_apply_patch`(envelope → `[(path, +content)]`) 추가 + `secret_scan`·`scope_check`·`suggest_compact`의 `_TARGET_TOOLS`에 `apply_patch` 추가 + path/content 추출 분기. 실 캡처 payload로 단위·핸들러 검증(보호경로 apply_patch→**exit 2 block**, `+`content fake key→**secret block**, Claude Edit 무회귀). **enforce 기제 = native sandbox/approval (확정, PreToolUse exit-2 아님)**: preflight-2(`CODEX-PREFLIGHT-2.md`) 정상 세션(`permission_mode=default`)에서도 exit-2가 apply_patch를 막지 못함(단 2회 모두 confound: ① bypass 모드 ② 훅이 code 1로 관측 + auto-accept 디버그 하니스). Codex가 approval 기반이라 apply_patch의 hard block은 **file-change approval(`FileChangeRequestApprovalResponse`: accept/decline)·sandbox·permission profile**이지 PreToolUse 훅 veto가 아님 → **Codex 포팅 hooks는 advisory(발화·로그·warn, hard block 아님)로 확정**(§4·AGENTS.md). 배선은 advisory를 정확화하는 데 유효. 스키마 사실: PermissionRequest 출력은 `{"decision":"decline"}` 거부, bare `"decline"` 수용. ② **route_nudge Claude 전제(미완)** — `Task tool`·`/gas`·`agent-routing.md` 참조 → custom-agent spawn 표현 재작성 필요. ③ **skill path(완화)** — preflight §6: Codex 0.141이 `~/.codex/skills` **와** `~/.agents/skills` **둘 다** 발견 → 현 `~/.codex/skills` 유효. ④ **hooks.json 스키마(OK)** — preflight §2: native와 일치, 수정 불요. ⑤ subagent depth-1 동작·depth-2 차단 확인(§5) → hub는 leaf 직접 spawn 평탄화(agent preamble 반영됨).
...
1. **`~/.codex/AGENTS.md`** (설치본) 상단 "Codex 환경 안전 노트": 포팅된 `secret_scan`·`scope_check` 훅은 advisory(발화/warn)이며 PreToolUse exit-2로 hard block 못 함(verified, `CODEX-PREFLIGHT-2.md`) — secret/scope 보호는 사용자 + Codex sandbox/approval 책임.
2. **본 문서** §1 D4 + §3.
