---
name: search-first
description: Research-before-coding workflow. Search for existing tools, libraries, and patterns before writing custom code. Invokes a general-purpose research agent.
origin: ECC
---
...
├─────────────────────────────────────────────┤
│  2. PARALLEL SEARCH (general-purpose)       │
│     ┌──────────┐ ┌──────────┐ ┌──────────┐  │
│     │  npm /   │ │  MCP /   │ │  GitHub / │  │
│     │  PyPI    │ │  Skills  │ │  Web      │  │
│     └──────────┘ └──────────┘ └──────────┘  │
...
2. Library / framework / API question? → **context7 MCP first** (version-current docs, backs CLAUDE.md §1.5); then check other MCPs via `~/.claude/settings.json`
3. Is there a skill for this? → Check `~/.claude/skills/`
4. Is there a GitHub implementation/template? → Run GitHub code search for maintained OSS before writing net-new code

### Full Mode (agent)

For non-trivial functionality, launch a general-purpose research agent:
...
### AI/LLM Integration
- Claude SDK → Context7 for latest docs (if the MCP is available — see preflight)
...
### Game Engine (Unreal)
- UE5 engine API/class → if engine MCP present, `lookup_docs` / `lookup_class` (UE5-indexed, see `MCP-UNREAL-SETUP.md` §9) before web; OSS libs → Context7

## Integration Points

### With planner agent
The planner should launch a general-purpose research agent before Phase 1 (Architecture Review):
