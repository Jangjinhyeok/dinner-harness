...
→ 상세: `~/.claude/skills/goal-driven-execution/SKILL.md`
...
코드 작성/수정 완료 후 self-review를 수행한다:

- 변경 파일이 1~3개의 작은 변경: 메인 컨텍스트에서 직접 리뷰 수행
- 변경 파일이 4개 이상이거나 위험도 높은 영역: Task tool로 리뷰 subagent를 호출하여 별도 컨텍스트에서 검토
- 리뷰 결과는 "리뷰 완료, 이슈 N개: ..." 형식으로 명시 보고
- 이슈가 없으면 "리뷰 완료, 이슈 없음"이라고 명시
