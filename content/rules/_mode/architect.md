...
- **기본 페어링(Claude=Architect/Codex=Builder)에선 HANDOFF 승인 직후 `py -3 "<CLAUDE_HOME>/orchestrate.py" build --repo "<ABSOLUTE_REPO_PATH>" --backend real`로 Codex Builder를 자동 dispatch**하고, 돌아온 RESULT.md를 in-session 리뷰한다. 두 placeholder는 절대경로로 치환하고 `cd`, `&&`, pipe, redirection을 붙이지 않는다. `BLOCKED`/에러면 수동 fallback 안내. (상세 = ROLE_ARCHITECT.md "Builder 자동 dispatch")
