...
3. Bash로 호출: `py -3 "<CLAUDE_HOME>/orchestrate.py" build --repo "<ABSOLUTE_REPO_PATH>" --backend real`
   - `<CLAUDE_HOME>`은 설치된 Claude home의 절대경로, `<ABSOLUTE_REPO_PATH>`는 원본 repository의 절대경로로 치환한다. `cd`, pipe, redirection을 붙이지 않는다. 이 정확한 command shape만 Claude permission allowlist가 허용한다. vendor/model/effort는 `content/routing.toml`의 active routing preset이 정한다(ADR-0020) — 특정 vendor를 명시 override하려면 `--builder claude`/`--builder codex`를 덧붙인다. 프리셋 전환은 `content/routing.toml`의 `preset` 값을 바꾸고 재설치하는 것뿐이다. 상세는 `~/.claude/rules/two-cli-reference.md`.
